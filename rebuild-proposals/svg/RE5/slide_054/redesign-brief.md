# Redesign-Brief · re5_src_054

- Modul: RE5
- Quellfolien: 54
- Kapitel/Lektion: 3/1
- Ausgabemodus: full_slide im module_redesign
- Referenz-Lock: RE4::1, RE4::13, RE4::48, RE4::65
- Archetyp: model-map
- Lernbotschaft: Nur ein belastbares Last-Lebensdauer-Modell erlaubt die sichere Extrapolation auf das Feld.
- Titelbehandlung: kein sichtbarer globaler Titel; Downstream-System ergänzt Titel, Regel, Footer, Logo und Szenenkennung.
- Inhaltsäquivalenz: Begriffe, Zahlen, Formeln, Plotbeziehungen und fachliche Zustände der Quellen 54 bleiben in der Zielkomposition erhalten.
- Spezialpfade: Plot=load_life_correlation; Formeln=keine; neue Piktogramme=keine.
- Assetentscheidung: technische Achsdiagramme aus Python, nicht triviale Formeln als lokale pfadbasierte Formel-SVGs, übrige Beziehungen nativ im SVG.
- Animation: animated umgesetzt; semantische Gruppen=load_life_plot, model_catalog, model_guardrail.
- QA: Quelle/Ziel/Referenz bei 1920×1080 und 960×540, beidseitiger Inhaltsabgleich, Textfit, Kontrast, Plot-/Formelgeometrie und Animationszustände.
