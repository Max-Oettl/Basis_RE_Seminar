...
def build_woehler(output: str | Path, data: dict[str, Any]) -> None:
    ...
    stress = np.asarray(data.get("stress_n_mm2", [768, 760, 755, 750, 746, 741, 646, 642, 640, 638, 635, 631, 548, 545, 542, 540, 536, 531, 458, 454, 451, 448, 445, 462]), dtype=float)
...
def build_histogram_density(output: str | Path, data: dict[str, Any]) -> None:
    ...
    density_area = ax_density.fill_between(x, 0, density, color=FUNCTION_COLORS["hazard"], alpha=0.12, zorder=2)
    density_area.set_gid("density_area")
    ...
    target_svg(
        ...
        sibling_groups=[
            ("histogram_bars", "Histogrammklassen der Stichprobe", bar_ids),
            ("density_curve", "Glatte Dichtefunktion der Grundgesamtheit", ["density_area", "density_curve_path"]),
        ],
    )
...
def build_woehler_3d(output: str | Path, data: dict[str, Any]) -> None:
    ...
    x = np.geomspace(5000, 1_000_000, 180)
    log_x = np.log10(x)
    segments = []
    peak_life = []
    peak_density = []
    for stress in stresses:
        mean_log = 6.05 - (stress - 420) / 235.0
        sigma = 0.14 + (740 - stress) / 2400.0
        density = np.exp(-0.5 * ((log_x - mean_log) / sigma) ** 2)
        density *= 0.92
        segments.append(np.column_stack([log_x, np.full_like(log_x, stress), density]))
        peak_life.append(mean_log)
        peak_density.append(float(density.max()))

    fig = plt.figure(figsize=(11.7, 6.5))
    clean_figure(fig)
    ax = fig.add_subplot(111, projection="3d")
    ax.set_facecolor("none")
    ridges = Line3DCollection(segments, colors=FUNCTION_COLORS["density"], linewidths=2.0, alpha=0.92)
    ridges.set_gid("density_ridges")
    ax.add_collection3d(ridges)
    line = ax.plot(peak_life, stresses, peak_density, color=FUNCTION_COLORS["hazard"], linewidth=4.2, zorder=8)[0]
    line.set_gid("woehler_peak_line")
    ...
    ax.set_zlabel("Ausfalldichte f(t)", labelpad=10)
...
def build_nkw_density(output: str | Path, data: dict[str, Any]) -> None:
    ...
    early = ax.fill_between(x, 0, density, where=x <= 0.55, color=FUNCTION_COLORS["hazard"], alpha=0.18)
    early.set_gid("early_failure_area_shape")
    early_label = ax.text(0.05, 0.98, "Ausfälle ab t = 0", color=FUNCTION_COLORS["hazard"], fontsize=14, weight="bold")
    early_label.set_gid("early_failure_label")
    ...
    target_svg(
        output,
        {
            ...
            "early_failure_area": "Frühe Ausfälle ab t gleich null",
        ...
        sibling_groups=[
            ...
            ("early_failure_area", "Frühe Ausfälle ab t gleich null", ["early_failure_area_shape", "early_failure_label"]),
...
def build_human_density(output: str | Path, data: dict[str, Any]) -> None:
    ...
    men_onset = ax.axvline(40, ymax=0.28, color="#244A7C", linewidth=1.8, linestyle=":")
    men_onset.set_gid("men_onset_line")
    men_onset_label = ax.text(39, 0.30, "Anstieg ab ≈ 40", color="#244A7C", fontsize=13, weight="bold", ha="right")
    men_onset_label.set_gid("men_onset_label")
    women_line = ax.plot(age, women, color=FUNCTION_COLORS["hazard"], linewidth=4.0, linestyle="--")[0]
    women_line.set_gid("women_density_path")
    women_onset = ax.axvline(50, ymax=0.31, color=FUNCTION_COLORS["hazard"], linewidth=1.8, linestyle=":")
    women_onset.set_gid("women_onset_line")
    women_onset_label = ax.text(51, 0.34, "Anstieg ab ≈ 50", color=FUNCTION_COLORS["hazard"], fontsize=13, weight="bold")
    women_onset_label.set_gid("women_onset_label")
    ...
    men_peak_label = ax.annotate("Männer ≈ 78 Jahre", xy=(men_location, 1.0), xytext=(61, 0.78), color="#244A7C", fontsize=14, weight="bold", arrowprops={"arrowstyle": "-", "color": "#244A7C", "linewidth": 1.8})
    ...
    women_peak_label = ax.annotate("Frauen ≈ 87 Jahre", xy=(women_location, 1.0), xytext=(92, 0.90), color=FUNCTION_COLORS["hazard"], fontsize=14, weight="bold", ha="right", arrowprops={"arrowstyle": "-", "color": FUNCTION_COLORS["hazard"], "linewidth": 1.8})
    ...
    target_svg(
        ...
        sibling_groups=[
            ("men_density", "Dichtefunktion Männer", ["men_density_path", "men_onset_line", "men_onset_label"]),
            ("women_density", "Dichtefunktion Frauen", ["women_density_path", "women_onset_line", "women_onset_label"]),
...
def build_human_cdf(output: str | Path, data: dict[str, Any]) -> None:
    ...
    ax.set_ylim(0, 100)
    men_line = ax.plot(age, men, color="#244A7C", linewidth=4.0)[0]
    men_line.set_gid("men_distribution")
    women_line = ax.plot(age, women, color=FUNCTION_COLORS["hazard"], linewidth=4.0, linestyle="--")[0]
    women_line.set_gid("women_distribution")
    ...
    age_80.set_gid("age_80_readout")
    ax.add_collection(age_80)
    ax.scatter([80, 80], [62, 36], s=100, color=["#244A7C", FUNCTION_COLORS["hazard"]], edgecolor="#FFFFFF", linewidth=2.0, zorder=6)
...
def build_reliability_partition(output: str | Path, data: dict[str, Any]) -> None:
    ...
    failed.set_gid("failure_probability_area")
    reliable = ax.fill_between(x, 0, density, where=x >= tx, color=FUNCTION_COLORS["reliability"], alpha=0.34)
    reliable.set_gid("reliability_area")
    ax.axvline(tx, color=FUNCTION_COLORS["hazard"], linewidth=2.0, linestyle="--")
    ax.text(tx, density.max() * 1.06, "$t_x$", color=FUNCTION_COLORS["hazard"], fontsize=16, weight="bold", ha="center")
...
def build_human_hazard(output: str | Path, data: dict[str, Any]) -> None:
    ...
    men_line = ax.plot(age, men, color="#244A7C", linewidth=4.0)[0]
    men_line.set_gid("men_hazard")
    women_line = ax.plot(age, women, color=FUNCTION_COLORS["hazard"], linewidth=4.0, linestyle="--")[0]
    women_line.set_gid("women_hazard")
...
def build_nkw_hazard(output: str | Path, data: dict[str, Any]) -> None:
    ...
    area.set_gid("wearout_area")
    ax.text(3.0, 2.05, "Verschleiß und Ermüdung", color=FUNCTION_COLORS["hazard"], fontsize=15, weight="bold")
