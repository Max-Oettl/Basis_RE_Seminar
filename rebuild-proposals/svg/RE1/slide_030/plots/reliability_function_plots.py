...
def build_woehler_3d(output: str | Path, data: dict[str, Any]) -> None:
    ...
    ax.set_box_aspect((1.8, 1.15, 0.9))
    ax.view_init(elev=24, azim=-128)
    ax.grid(True)
    line_label = ax.text2D(0.73, 0.86, "Wöhlerlinie", transform=ax.transAxes, color=FUNCTION_COLORS["hazard"], fontsize=16, weight="bold")
    ...
    target_svg(
        ...
        sibling_groups=[
            ("woehler_peak_line", "Wöhlerlinie über den Dichtemaxima", ["woehler_peak_line", "woehler_peak_label"]),
        ],
    )
...
def build_human_density(output: str | Path, data: dict[str, Any]) -> None:
    ...
    women_peak_label = ax.annotate("Frauen · 87 Jahre", xy=(women_location, 1.0), xytext=(96, 0.88), color=FUNCTION_COLORS["hazard"], fontsize=14, weight="bold", ha="right", arrowprops={"arrowstyle": "-", "color": FUNCTION_COLORS["hazard"], "linewidth": 1.8})
