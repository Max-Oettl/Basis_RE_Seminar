...
def build_human_density(output: str | Path, data: dict[str, Any]) -> None:
    ...
    men_marker = ax.scatter([men_location], [men_peak_height], s=100, color="#244A7C", edgecolor="#FFFFFF", linewidth=2.0, zorder=5)
    men_marker.set_gid("men_peak_marker")
    men_peak_label = ax.annotate("Männer · 78 Jahre", xy=(men_location, men_peak_height), xytext=(68, 0.60), color="#244A7C", fontsize=14, weight="bold", ha="right", arrowprops={"arrowstyle": "-", "color": "#244A7C", "linewidth": 1.8})
...
def build_empirical_cdf(output: str | Path, data: dict[str, Any]) -> None:
    ...
    right_title = right.set_title("Kumulierte Häufigkeit bis t", color=FUNCTION_COLORS["failure_probability"], fontsize=17, weight="bold", pad=12)
