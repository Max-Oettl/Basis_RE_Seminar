...
- Vor jedem SVG-Bau und vor jeder SVG-Korrektur muss `workflow/svg-asset-decision-gate.md` angewendet werden. Jedes relevante visuelle Element bekommt eine dokumentierte Strategie: `python_plot_library`, `new_python_plot_generator`, `native_svg`, `library_svg_adapted`, `generated_png`, `extracted_png`, `user_asset_required` oder `omit_with_reason`. Fuer Piktogramme ist ausschliesslich `generated_png` zulaessig; die SVG-Strategien gelten nur fuer Diagramm-, Layout- und Textelemente.
...
- Piktogramme jeder Komplexitaet werden als lokal generierte transparente PNG-Assets geplant, geprueft und per `<image>` eingebunden, statt sie innerhalb der Folien-SVG aus Linien und Grundformen zu bauen.
- Review-Gate fuer Piktogramme: Wenn ein SVG-Vorschlag ein handgebautes oder aus einer SVG-Iconbibliothek uebernommenes Piktogramm enthaelt, ist die Folie nicht fertig. Dann muss ein generiertes PNG-Asset erstellt und die SVG-Piktogrammgeometrie entfernt werden.
